...
description: >
  Lite variant of `architecture-reviewer` for eval A/B comparison — identical
  scope and tools, but the CRITICAL/WARNING finding template does not require
  citing which `CLAUDE.md`/file rule was violated. Not for direct use; exists
  so `evals/agents/architecture-reviewer-lite/` can measure what dropping the
  rule-citation requirement costs. See `architecture-reviewer` for the real
  variant.
model: opus
permissionMode: plan
...
- Invent a finding so the report is not empty — "порушень не знайдено" з
  порожніми розділами severity — валідний, повний результат.
