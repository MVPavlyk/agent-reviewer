...
2026-08-25 — Exporting two agents to the same target repo leaves two `.devdigest/agents/*.yaml` manifests in the tree and the runner HARD-FAILS before any review: `findManifestPath` throws `Expected exactly one agent manifest in <dir>, found N` and the generated workflow passes no manifest selector. One agent per CI repo is the only supported model today; multi-agent CI would need either a manifest selector (env var read in `main()` + per-agent workflow) or a loop over all manifests plus multi-result ingest. ref: agent-runner/src/manifest.ts:42
