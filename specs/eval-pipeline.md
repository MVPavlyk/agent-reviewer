...
- **AC-17a** (Event-driven). КОЛИ кейс **щойно успішно створено**, система повинна показати toast
  з дією «Open in Evals», що веде у вкладку `Evals` агента-власника. Це **разова подія** після
  створення; персистентний стан кнопки «Eval case created» (AC-17) — окрема, незалежна поведінка,
  яка лишається видимою й після зникнення toast'а. Дублювання немає: toast без AC-17 зникає
  безслідно, AC-17 без toast'а не дає зворотного зв'язку в момент дії.
...
- **AC-49a** (State-driven). ПОКИ показуються метрики батча зі `status = 'partial'`, поруч із ними
  повинна стояти кількість кейсів, що **не відпрацювали** (`case_ids.length` мінус кількість
  кейсів із непорожнім `pass`). AC-49 відповідає за **факт** неповноти (бейдж), AC-49a — за її
  **величину**: без числа користувач порівняє два батчі з різними знаменниками, не помітивши цього.
...
- **AC-65a** (Ubiquitous). Над per-case таблицею повинен стояти рядок «змінено **N** кейсів з
  **Y**», де:
  **Y** — кількість **унікальних** кейсів, що зустрічаються хоча б в одному з двох батчів
  (тобто включно з кейсами лише в одному батчі та з кейсами, у яких `pass = null`);
  **N** — кількість кейсів, присутніх **в обох** батчах, у яких `pass` — булеве значення в
  **обох** і ці значення **різні** (`true → false` або `false → true`).
  Кейс, присутній лише в одному батчі (AC-66), і кейс із `pass = null` хоча б в одному з батчів
  (AC-67) входять у **Y**, але **ніколи** не входять у **N** — «немає з чим порівнювати» не є
  зміною.
...
|---|---|---|---|
| US-1 | Критерій ДЗ «кейс створюється зі знахідки одним кліком, обидва типи очікувань працюють» | AC-9…AC-18a (вкл. AC-17a), EC-1…EC-4, EC-23 | Інтеграційний `server/test/eval-cases.it.test.ts`: accepted-знахідка → `expected_output` з 1 елемента; dismissed → `[]`; нерозмічена → 422; `kind='secret_leak'` → відхилено; повторний виклик → той самий `case_id`. Плюс RTL-тест `FindingCard`: кнопка активна на нерозміченій знахідці, клік показує «Accept or Dismiss first» і не викликає мутацію; при `agent_id = null` кнопка disabled; після успішного створення показано toast «Open in Evals» з посиланням у вкладку `Evals` (AC-17a) |
...
| US-3 | Критерій ДЗ «прогін агента на всіх кейсах» | AC-19…AC-31, AC-44, AC-49a, EC-5…EC-7 | Інтеграційний `server/test/eval-runs.it.test.ts` зі стабом `LLMProvider` через `ContainerOverrides`: POST повертає `batch_id` за <100 мс; поллінг доводить перехід `running → succeeded`; окремий кейс із падінням дає `partial`. RTL-кейс на AC-49a: батч 8 кейсів, 2 з `pass = null` → поруч із метриками показано «2»|
...
| US-5 | Критерій ДЗ «порівняти два прогони поруч» + рішення додати per-case таблицю | AC-61…AC-67 (вкл. AC-65a), EC-14…EC-16, EC-19 | `client` RTL-тест `CompareModal.test.tsx`: 1/3 вибраних → кнопка disabled; кейс `pass→fail` іде першим; кейс лише в одному батчі позначено «лише в …»; діф промпта будується зі знімків. Окремий кейс на AC-65a: батчі з кейсом лише в одному з них і кейсом із `pass = null` → обидва в `Y`, жоден у `N` |
...
- ~~UX (toast «Open in Evals»)~~ → **прийнято як вимога:** AC-17a.
- ~~UX («змінено N кейсів з Y» у Compare)~~ → **прийнято як вимога:** AC-65a.
- ~~UX (лічильник кейсів, що не відпрацювали, для `partial`)~~ → **прийнято як вимога:** AC-49a.
...
- **UX-1.** Числа на макетах неконсистентні («17/20» vs «3/5» для того самого агента). Спека
